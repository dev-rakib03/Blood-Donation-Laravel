      ...
      <div class="dashboard-list-box table-responsive invoices with-icons">
        ...
        <thead>
          <tr>
            <th>SL</th>
            <th>Description</th>
            <th class="hide_767_text">Needed Blood Group</th>
          ...
          <tr>
            ...
            <td>
              <span class="hide_767_text">
                <strong>{{$my_req->name}}</strong> requesting for blood.
            ...
            <td>
              <div style="display: inline-flex; border: none; vertical-align: middle;">
                <span class="chat-image-table" title="massage"><img style="cursor: pointer;" src="{{asset('/')}}frontend/images/chat-icon.png" onclick="window.location='/message/chatbox/{{$my_req->user_id}}';"></span>&nbsp;
                @if($my_req->status!=1 && $my_req->status!=0)
                <label  onclick="accpet_request('{{$my_req->id}}');" class="actionbutton" style="color: green;"><i class="fa fa-check"></i></label>&nbsp;
                <label onclick="cancle_request('{{$my_req->id}}');" class="actionbutton"><i class="fa fa-close"></i></label>
                @endif
              </div>              
            </td>
          </tr>
          ...
          <center>
            <div class="description-popup" id="desc_popup{{$my_req->id}}">
              <label style="float: right;  margin: 5px;  margin-top: 0px;" onclick="document.getElementById('desc_popup{{$my_req->id}}').style.display='none'"><i class="fa fa-close"></i>
                ...
                <br>Phone: {{$my_req->phone}}
                @endif
                <br>Needed Blood Group: {{$my_req->blood_group}}
                <br>Needed Date: {{date("d M Y", strtotime($my_req->needed_date))}}<br>
                @if($my_req->status==2)
                  <span class="badge badge-pill badge-danger text-uppercase">Pending</span>
                @endif
                @if($my_req->status==1)
...
<script type="text/javascript">
  function cancle_request(id) { 
    alertify.confirm('Blood Request',"<center><h3>Are you sure?</h3>you want to cancle blood request?<center>",
    function(){
      //alertify.success('Ok');
      window.location.href = '{{asset("/")}}cancle-blood-request/'+id;
    },
    function(){
  ...
  function accpet_request(id) { 
    alertify.confirm('Blood Request',"<center><h3>Are you sure?</h3>you want to accpet blood request?<center>",
    function(){
      //alertify.success('Ok');
      window.location.href = '{{asset("/")}}accpet-blood-request/'+id;
    },
    function(){
      alertify.error('Cancel');
    });
